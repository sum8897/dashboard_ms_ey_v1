...
@Component({
  selector: 'app-ques-average-barchart',
  templateUrl: './ques-average-barchart.component.html',
  styleUrls: ['./ques-average-barchart.component.scss']
})
export class QuesAverageBarchartComponent implements OnInit, OnDestroy {
  ...
  config;
  data;
  fileName: string = "% Average Score";
  reportName: string = 'ques_average_barchart';
  filters: any = [];
  levels: any;
  tableReportData: any;
  minDate: any;
     ...
     if (this.drillDownDetails !== undefined) {
      ...
      Object.keys(queries).forEach(async (key: any) => {
        ...
        if (query && key === 'table'&& filterneed) {
          ...
          // this.config = config;
          // console.log('tablereport',this.tableReportData)
