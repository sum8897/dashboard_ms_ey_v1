...
import { config } from "src/app/views/staff/config/staff_config";
import { ManagementTabComponent } from "../../management-tab.component";

@Component({
  selector: "app-table-department-two",
  templateUrl: "./table-department-two.component.html",
  styleUrls: ["./table-department-two.component.scss"],
})
export class TableDepartmentTwoComponent implements OnInit {
  reportName: string = "management_second_table";
  filters: any = [];
  levels: any;
  tableReportData: any;
  bigNumberReportData: any = {
    reportName: "Management Second Table",
  };
  minDate: any;
  maxDate: any;
  ...
  constructor(
    ...
    private csv: ManagementTabComponent,
  ...
  ngOnInit(): void {
    let {
      options: { searchBar_config },
    } = config[this.reportName];
    this.searchFilterConfig = {
      ...searchBar_config,
    };
    this.drillDownSubscription =
      this._reportDrilldownService.drilldownData.subscribe(async (data) => {
        if (
          data &&
