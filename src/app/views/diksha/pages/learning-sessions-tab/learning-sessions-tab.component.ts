...
constructor(private _wrapperService: WrapperService, private _rbacService: RbacService) {
    ...
    if (this.hasCommonFilters) {
        ...
        this.learningSessions?.getReportData({ filterValues: this.filters.map((filter) => { return { columnName: filter.id, value: filter.value, options: filter.options} }) });
        }
    ...
    this.reportsData = [];
    this.learningSessions?.getReportData({ filterValues: filters.map((filter) => { return { columnName: filter.id, value: filter.value, options: filter.options} }) });
        }
