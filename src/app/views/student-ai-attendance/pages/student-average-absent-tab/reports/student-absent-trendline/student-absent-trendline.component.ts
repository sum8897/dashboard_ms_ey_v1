...
@Component({
  selector: "app-student-absent-trendline",
  templateUrl: "./student-absent-trendline.component.html",
  styleUrls: ["./student-absent-trendline.component.scss"],
})
export class StudentAbsentTrendlineComponent implements OnInit, OnDestroy {
      ...
      Object.keys(queries).forEach(async (key: any) => {
        ...
        if (query && key === "table" && filterneed) {
          ...
          let { reportData, config } =
            await this._dataService.getLineChartReportData(
              query,
              options,
              filters,
              defaultLevel
