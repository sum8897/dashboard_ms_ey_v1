...
constructor(private _wrapperService: WrapperService, private _rbacService: RbacService) {
    ...
    if (this.hasCommonFilters) {
        ...
        this.courseWiseStatus?.getReportData({ filterValues: this.filters.map((filter) => { return { columnName: filter.valueProp, value: filter.value } }) });
        }
    ...
    this.reportsData = [];
    this.courseWiseStatus?.getReportData({ filterValues: filters.map((filter) => { return { columnName: filter.valueProp, value: filter.value } }) });
        }
