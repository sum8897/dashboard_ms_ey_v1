...
import { GrossEnrollTableComponent } from './reports/gross-enroll-table/gross-enroll-table.component';
import { NetEnrollTableComponent } from './reports/net-enroll-table/net-enroll-table.component';
import { GenderParityBarchartComponent } from './reports/gender-parity-barchart/gender-parity-barchart.component';
import moment from 'moment';

@Component({
  selector: 'app-key-indicators-tab',
  templateUrl: './key-indicators-tab.component.html',
  styleUrls: ['./key-indicators-tab.component.scss']
})
export class KeyIndicatorsTabComponent implements OnInit, OnDestroy {
...
bigNumberMetrics: any = [];
  ...
  //
  @ViewChild('grossEnrollTable') grossEnrollTable: GrossEnrollTableComponent;
  @ViewChild('genderParityBarchart') genderParityBarchart: GenderParityBarchartComponent;
 
  @ViewChild('netEnrollTable') netEnrollTable: NetEnrollTableComponent;
  ...
  async ngAfterViewInit(): Promise<void> {
      if (this.hasCommonFilters) {
          ...
          this.grossEnrollTable?.getReportData({filterneed: this.hasCommonFilters, filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) },this.startDate,this.endDate);

          this.netEnrollTable?.getReportData({filterneed: this.hasCommonFilters, filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) },this.startDate,this.endDate);

          
        
         
          this.genderParityBarchart?.getReportData({filterneed: this.hasCommonFilters, filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) },this.startDate,this.endDate);
