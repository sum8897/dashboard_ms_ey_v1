      ...
      filters.every((filter: any) => {
        if (Number(this.rbacDetails?.role) === Number(filter.hierarchyLevel)) {
          ...
          Object.keys(queries).forEach((key) => {
            ...
            timeSeriesQueries[key] = this.parseRbacFilter(
  ...
  applyCriteria(data: any) {
    ...
    if (data && this.backUpData.length > 0) {
      let filteredData = this.backUpData.filter((row: any) => {
        let value = row?.[data.unitKey]?.value
          ? row?.[data.unitKey]?.value
          : row?.[data.unitKey];
        return (
          Number(data?.fromRange) <= Number(value) &&
          Number(value) <= Number(data?.toRange)
        );
      });
      this.tableReportData = {
        ...this.tableReportData,
