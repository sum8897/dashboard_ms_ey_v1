...
import { DataService } from 'src/app/core/services/data.service';import { config } from 'src/app/views/teacher-attendance/config/teacher_attendance_config';
import { TeacherSummaryTabComponent } from '../../teacher-summary-tab.component';

@Component({
  selector: 'app-teacher-trendline',
  templateUrl: './teacher-trendline.component.html',
  styleUrls: ['./teacher-trendline.component.scss']
})
export class TeacherTrendlineComponent implements OnInit, OnDestroy {
  ...
  data;
  fileName: string = "";
  reportName: string = 'teacher_trendchart';
  filters: any = [];
  levels: any;
  tableReportData: any;
  minDate: any;
     ...
     if (this.drillDownDetails !== undefined) {
      ...
      Object.keys(queries).forEach(async (key: any) => {
        ...
        if (query && key === 'table'&& filterneed) {
          ...
          let { reportData, config } = await this._dataService.getLineChartReportData(query, options, filters, defaultLevel);
