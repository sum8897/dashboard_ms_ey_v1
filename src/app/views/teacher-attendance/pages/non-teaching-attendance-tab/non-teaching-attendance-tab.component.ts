...
import { NonTeachingMapComponent } from './reports/non-teaching-map/non-teaching-map.component';
import { config } from 'src/app/views/teacher-attendance/config/teacher_attendance_config';
import moment from 'moment';
@Component({
  selector: 'app-non-teaching-attendance-tab',
  templateUrl: './non-teaching-attendance-tab.component.html',
  styleUrls: ['./non-teaching-attendance-tab.component.scss']
})
export class NonTeachingAttendanceTabComponent implements OnInit, AfterViewInit {
  ...
  tabLabel:any='Map View of Non Teaching Staff Attendance';
    // tabName:any ='Student Performance';

  @ViewChild('nonteachingmap') nonteachingmap:NonTeachingMapComponent
  ...
  async ngAfterViewInit(): Promise<void> {
    if (this.hasCommonFilters) {
        ...
        this.nonteachingmap?.getReportData({filterneed: this.hasCommonFilters, filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) },this.startDate,this.endDate);
        }
