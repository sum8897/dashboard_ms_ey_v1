...
import { config } from 'src/app/views/smart-classroom/config/smart_classroom_config';
import { SmartTableComponent } from 'src/app/views/smart-classroom/pages/ict-classroom-tab/reports/smart-table/smart-table.component';

import { SmartCompleteBignumberComponent } from 'src/app/views/smart-classroom/pages/ict-classroom-tab/reports/smart-complete-bignumber/smart-complete-bignumber.component';
import { SmartIncompleteBignumberComponent } from 'src/app/views/smart-classroom/pages/ict-classroom-tab/reports/smart-incomplete-bignumber/smart-incomplete-bignumber.component';
import moment from 'moment';
@Component({
  selector: 'app-ict-classroom-tab',
  templateUrl: './ict-classroom-tab.component.html',
  styleUrls: ['./ict-classroom-tab.component.scss']
})
export class IctClassroomTabComponent implements OnInit, OnDestroy {
...
hasCommonFilters: boolean = true;
  ...
  @ViewChild('smartTable') smartTable: SmartTableComponent;
  @ViewChild('smartCompleteBigNumber') smartCompleteBigNumber: SmartCompleteBignumberComponent;
  @ViewChild('smartNonCompleteBigNumber') smartNonCompleteBigNumber: SmartIncompleteBignumberComponent;
  ...
  async ngAfterViewInit(): Promise<void> {
      if (this.hasCommonFilters) {
          ...
          this.smartTable?.getReportData({filterneed: this.hasCommonFilters, filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) },this.startDate,this.endDate);
        
          this.smartCompleteBigNumber?.getReportData({filterneed: this.hasCommonFilters, filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) },this.startDate,this.endDate);

          this.smartNonCompleteBigNumber?.getReportData({filterneed: this.hasCommonFilters, filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) },this.startDate,this.endDate);
