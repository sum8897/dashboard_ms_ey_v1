...
export class ReviewMeetingsTabComponent implements OnInit, AfterViewInit {
  ...
  async ngAfterViewInit(): Promise<void> {
    if (this.hasCommonFilters) {
      ...
      this.reviewMeetingsConducted?.getReportData({ filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) });
      this.reviewMeetingsStatus?.getReportData({ filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) });
      this.reviewMeetingsConductedBignumber?.getReportData({ filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) });
    }
  ...
  filtersUpdated(filters: any) {
    this.reportsData = [];
    this.reviewMeetingsConducted?.getReportData({ filterValues: filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) });
    this.reviewMeetingsStatus?.getReportData({ filterValues: filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) });
    this.reviewMeetingsConductedBignumber?.getReportData({ filterValues: filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) });
  }
