        ...
        "filters": [
            {
                ...
                "hierarchyLevel": "1",
                "timeSeriesQueries": {
                    "table":"select ceil(count(t1.school_id)/count(t2.school_id)*100) as compliance_percentage, t2.district_id, t2.district_name from (select c.percentage, c.school_id as school_id from  (select a.school_id,(sum(a.sum)/sum(b.sum))*100 as percentage, sum(b.sum) as total_students, sum(a.sum) as attendace_marked from datasets.school_attendance_totalstudent_daily_school as b join datasets.school_attendance_studentsmarked_daily_school as a on a.school_id = b.school_id and a.date = b.date where a.date between startDate and endDate group by a.school_id) as c where c.percentage > 50) as t1 join (select d.school_id, e.district_id, e.district_name from datasets.school_attendance_studentsmarked_daily_school as d join dimensions.school as e on d.school_id = e.school_id group by d.school_id, e.district_id, e.district_name) as t2 on t1.school_id = t2.school_id group by t2.district_id, t2.district_name",
                    },
                "actions": {
                    "queries": {
            ...
            },
            {
                ...
                "hierarchyLevel": "2",
                "timeSeriesQueries": {
                    "table":"select ceil(count(t1.school_id)/count(t2.school_id)*100) as compliance_percentage, t2.block_id, t2.block_name from (select c.percentage, c.school_id as school_id from (select a.school_id,(sum(a.sum)/sum(b.sum))*100 as percentage, sum(b.sum) as total_students, sum(a.sum) as attendace_marked from datasets.school_attendance_totalstudent_daily_school as b join datasets.school_attendance_studentsmarked_daily_school as a on a.school_id = b.school_id and a.date = b.date where a.date between startDate and endDate group by a.school_id) as c where c.percentage > 50) as t1 join (select d.school_id, e.block_id, e.block_name from datasets.school_attendance_studentsmarked_daily_school as d join dimensions.school as e on d.school_id = e.school_id group by d.school_id, e.block_id, e.block_name) as t2 on t1.school_id = t2.school_id group by t2.block_id, t2.block_name"
                },
                "actions": {
                    "queries": {
            ...
            },
            {
                ...
                "hierarchyLevel": "3",
                "timeSeriesQueries": {
                    "table":"select ceil(count(t1.school_id)/count(t2.school_id)*100) as compliance_percentage, t2.cluster_id, t2.cluster_name from (select c.percentage, c.school_id as school_id from (select a.school_id,(sum(a.sum)/sum(b.sum))*100 as percentage, sum(b.sum) as total_students, sum(a.sum) as attendace_marked from datasets.school_attendance_totalstudent_daily_school as b join datasets.school_attendance_studentsmarked_daily_school as a on a.school_id = b.school_id and a.date = b.date where a.date between startDate and endDate group by a.school_id) as c where c.percentage > 50) as t1 join (select d.school_id, e.cluster_id, e.cluster_name from datasets.school_attendance_studentsmarked_daily_school as d join dimensions.school as e on d.school_id = e.school_id group by d.school_id, e.cluster_id, e.cluster_name) as t2 on t1.school_id = t2.school_id group by t2.cluster_id, t2.cluster_name"
                },
                "actions": {
                    "queries": {
            ...
            },
            {
                ...
                "hierarchyLevel": "4",
                "timeSeriesQueries": {
                    "table":"select  e.school_name ,ceil(round(sum(a.sum)/sum(b.sum)*100))as compliance_percentage, sum(b.sum) as total_students, sum(a.sum) as attendace_marked from datasets.school_attendance_totalstudent_daily_school as b join datasets.school_attendance_studentsmarked_daily_school as a on a.school_id = b.school_id and a.date = b.date join dimensions.school as e on a.school_id = e.school_id where a.date between startDate and endDate group by a.school_id,e.school_name"
                    },
                "actions": {
                    "queries": {
        ...
        "filters": [
            {
                ...
                "hierarchyLevel": "1",
                "timeSeriesQueries": {
                    "bigNumber":"select ceil(count(t1.school_id)/count(t2.school_id)*100) as compliance_percentage from (select c.percentage, c.school_id as school_id from  (select a.school_id,(sum(a.sum)/sum(b.sum))*100 as percentage, sum(b.sum) as total_students, sum(a.sum) as attendace_marked from datasets.school_attendance_totalstudent_daily_school as b join datasets.school_attendance_studentsmarked_daily_school as a on a.school_id = b.school_id and a.date = b.date where a.date between startDate and endDate group by a.school_id) as c where c.percentage > 50) as t1 join (select d.school_id, e.district_id, e.district_name from datasets.school_attendance_studentsmarked_daily_school as d join dimensions.school as e on d.school_id = e.school_id group by d.school_id, e.district_id, e.district_name) as t2 on t1.school_id = t2.school_id "
                },
                "actions": {
                    "queries": {
            ...
            },
            {
                ...
                "hierarchyLevel": "4",
                "timeSeriesQueries": {
                    "bigNumber":"select ceil(round(sum(a.sum)/sum(b.sum)*100))as compliance_percentage, sum(b.sum) as total_students, sum(a.sum) as attendace_marked from datasets.school_attendance_totalstudent_daily_school as b join datasets.school_attendance_studentsmarked_daily_school as a on a.school_id = b.school_id and a.date = b.date join dimensions.school as e on a.school_id = e.school_id where a.date between startDate and endDate"
                },
                "actions": {
                    "queries": {
    ...
    sac_attendance_compliance_rank: {
        ...
        "filters": [
            ...
            },
            {
                ...
                "hierarchyLevel": "5",
                "timeSeriesQueries": {
                    "table":"select  e.school_name ,ceil(round(sum(a.sum)/sum(b.sum)*100))as compliance_percentage,dense_rank() over(order by ceil(round(sum(a.sum)/sum(b.sum)*100)) desc) as rank, sum(b.sum) as total_students, sum(a.sum) as attendace_marked from datasets.school_attendance_totalstudent_daily_school as b join datasets.school_attendance_studentsmarked_daily_school as a on a.school_id = b.school_id and a.date = b.date join dimensions.school as e on a.school_id = e.school_id where a.date between startDate and endDate group by a.school_id,e.school_name"
                },
                "actions": {
                    "queries": {
