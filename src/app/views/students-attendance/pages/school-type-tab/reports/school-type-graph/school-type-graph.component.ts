...
import {
  getBarDatasetConfig,
  getChartJSConfig,
} from "src/app/core/config/ChartjsConfig";
...
@Component({
  selector: "app-school-type-graph",
  templateUrl: "./school-type-graph.component.html",
  styleUrls: ["./school-type-graph.component.scss"],
})
export class SchoolTypeGraphComponent implements OnInit {
  ...
  constructor(
    ...
    private csv: SchoolTypeTabComponent
  ) {
  ...
  ngOnInit(): void {
    ...
    this._criteriaService.criteriaObject.subscribe((data) => {
      if (data && data?.linkedReports?.includes(this.reportName)) {
        ...
        this.tableReportData = this._criteriaService.applyCriteria(
          data,
          this.backUpData,
    ...
    this.benchmarkSubscription =
      this._benchmarkService.benchmarkValues.subscribe((values: any) => {
        let {
          options: {
            barChart: { benchmarkConfig },
          },
        } = config[this.reportName];
        if (
          values &&
          Object.keys(values).includes(benchmarkConfig?.linkedReport)
        ) {
  ...
  async getReportData(
    ...
    let { filterValues, timeSeriesValues, filterneed } = values ?? {
      filterValues: [],
      timeSeriesValues: [],
      filterneed: [],
    };
    ...
    if (this.drillDownDetails !== undefined) {
      let result: any = await this._reportDrilldownService.drilldown(
        { hierarchyLevel: this.drillDownLevel },
        this.rbacDetails,
        config[this.reportName],
        startDate,
        endDate,
      ...
      if (this.rbacDetails?.role) {
        filters.every((filter: any) => {
          if (
            Number(this.rbacDetails?.role) === Number(filter.hierarchyLevel)
          ) {
      ...
      Object.keys(queries).forEach(async (key: any) => {
        ...
        if (query && key === "table" && filterneed) {
          this.tableReportData = await this._dataService.getTableReportData(
            query,
            options
          );
          if (this.tableReportData?.data?.length > 0) {
          ...
          this.tableReportData = await this._dataService.getBigNumberReportData(
            query,
            options,
            "averagePercentage",
            this.tableReportData
          ...
          // this._dataService.stackBar(reportData,config,"male","Male",'rgba(255, 0, 0, 0.5)',0.42);
          this._dataService.stackBar(
            reportData,
            config,
            "male_absent",
            "Male Absent",
          ...
          if (this.tableReportData?.values?.length > 0) {
            let reportsData = {
              reportData: this.tableReportData.values,
          ...
          this.tableReportData = await this._dataService.getMapReportData(
            query,
            options,
            metricFilter
          );
  ...
  getBarChartReportData(query, options, filters, defaultLevel): void {
    ...
    this._commonService.getReportDataNew(query).subscribe((res: any) => {
      ...
      // this._dataService.stackBar(this.tableReportData,this.config,"male","Male",'rgba(255, 0, 0, 0.5)',0.38);
      this._dataService.stackBar(
        this.tableReportData,
        this.config,
        "male_absent",
      ...
      let subscription = this._benchmarkService.benchmarkValues.subscribe(
        (values) => {
          if (
            values &&
            Object.keys(values).includes(benchmarkConfig?.linkedReport) &&
            this.benchmarkValues?.index &&
            values.index == this.benchmarkValues.index
          ) {
            setTimeout(() => {
  ...
  getConfig() {
    ...
    let currentLevel = this.drillDownLevel
      ? this.drillDownLevel
      : this.rbacDetails.role;
    if (reportValues) {
      annotations = Object.keys(reportValues)
    ...
    this.tableReportData.values.forEach((row) => {
      let tooltip = this._wrapperService.constructTooltip(
        tooltipMetrics,
        row,
        metricValueProp,
        "barChart"
    ...
    let barChartConfig = getChartJSConfig({
      ...
      datasets: getBarDatasetConfig([
        {
          dataExpr: metricValueProp,
          label: metricLabelProp,
        },
      ]),
  ...
  async drilldownData(event: any) {
    ...
    switch (Number(hierarchyLevel)) {
      ...
      case 5:
        drillDownDetails = {
          ...this.rbacDetails,
          cluster: id ? id : this.drillDownDetails.school,
        };
        break;
    }
      ...
      filters.every((filter: any) => {
        if (Number(hierarchyLevel) === Number(filter.hierarchyLevel)) {
          ...
          Object.keys(queries).forEach((key) => {
            ...
            timeSeriesQueries[key] = parseRbacFilter(
              timeSeriesQueries[key],
              drillDownDetails
            );
          });
          return false;
    ...
    Object.keys(queries).forEach((key: any) => {
      if (
        ...
        Object.keys(timeSeriesQueries).length > 0
      ) {
        onLoadQuery = parseTimeSeriesQuery(
          timeSeriesQueries[key],
          this.startDate,
