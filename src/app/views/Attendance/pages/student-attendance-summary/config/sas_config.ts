export const config = {
    ...
    gender_wise_average_attendance: {
    ...
    grade_wise_average_attendance: {
        "label": "Grade Wise Average Attendance",
        "queries": {
            "barChart": "select min(date) as min_date, max(date) as max_date, state_name as location,round(avg(percentage),2) as percentage from ingestion.sac_stds_avg_atd_grade_wise_by_state as t left join ingestion.dimension_state as s on t.state_id = s.state_id group by t.state_id, state_name",
        },
        "timeSeriesQueries": {
            "barChart": "select state_name as location,round(avg(percentage),2) as percentage from ingestion.sac_stds_avg_atd_grade_wise_by_state as t left join ingestion.dimension_state as s on t.state_id = s.state_id where date between startDate and endDate group by t.state_id, state_name",
        },
        "defaultLevel": "state",
        "filters": [
            {
                ...
                "valueProp": "state_id",
                "hierarchyLevel": "1",
                "query": "select t.state_id, state_name from ingestion.sac_stds_avg_atd_grade_wise_by_state as t left join ingestion.dimension_state as s on t.state_id = s.state_id group by t.state_id,state_name",
                "timeSeriesQueries": {
                    "barChart": "select district_name as location, round(avg(percentage),2) as percentage from ingestion.sac_stds_avg_atd_grade_wise_by_district as t left join ingestion.dimension_district as d on t.district_id = d.district_id left join ingestion.dimension_master as m on t.district_id = m.district_id where (date between startDate and endDate) and m.state_id={state_id} group by t.district_id,district_name",
                },
                "actions": {
                    "queries": {
                        "barChart": "select min(date) as min_date, max(date) as max_date, district_name as location, state_name,round(avg(percentage),2) as percentage from ingestion.sac_stds_avg_atd_grade_wise_by_district as t left join ingestion.dimension_master as m on t.district_id = m.district_id left join ingestion.dimension_master as m on t.district_id = m.district_id where m.state_id={state_id} group by t.district_id,district_name",
                    },
                    "level": "district"
                }
            },
            {
                ...
                "hierarchyLevel": "2",
                "timeSeriesQueries": {
                    "barChart": "select block_name as location, district_name, state_name,round(avg(percentage),2) as percentage from ingestion.sac_stds_avg_atd_grade_wise_by_block as t left join ingestion.dimension_block as b on t.block_id = t2.block_id left join ingestion.dimension_master as m on t.block_id = m.block_id where (date between startDate and endDate) and m.district_id={district_id} group by t.block_id,block_name",
                },
                "query": "select t.district_id, district_name from ingestion.sac_stds_avg_atd_grade_wise_by_district as t left join ingestion.dimension_district as d on t.district_id = d.district_id left join ingestion.dimension_master as m on t.district_id = m.district_id where m.state_id={state_id} group by t.district_id,district_name",
                "actions": {
                    "queries": {
                        "barChart": "select min(date) as min_date, max(date) as max_date, block_name as location,round(avg(percentage),2) as percentage from ingestion.sac_stds_avg_atd_grade_wise_by_block as t left join ingestion.dimension_block as b on t.block_id = b.block_id left join ingestion.dimension_master as m on t.block_id = m.block_id where m.district_id={district_id} group by t.block_id,block_name",
                    },
            ...
            },
            {
                ...
                "hierarchyLevel": "3",
                "timeSeriesQueries": {
                    "barChart": "select cluster_name as location, block_name, district_name, state_name,round(avg(percentage),2) as percentage from ingestion.sac_stds_avg_atd_grade_wise_by_cluster as t left join ingestion.dimension_cluster as c on t.cluster_id = c.cluster_id left join ingestion.dimension_master as m on t.cluster_id = m.cluster_id where (date between startDate and endDate) and m.block_id={block_id} group by t.cluster_id,cluster_name",
                },
                "query": "select t.block_id, block_name from ingestion.sac_stds_avg_atd_grade_wise_by_block as t left join ingestion.dimension_block as b on t.block_id = b.block_id left join ingestion.dimension_master as m on t.block_id = m.block_id where m.district_id={district_id} group by t.block_id,block_name",
                "actions": {
                    "queries": {
                        "barChart": "select min(date) as min_date, max(date) as max_date, cluster_name as location, round(avg(percentage),2) as percentage from ingestion.sac_stds_avg_atd_grade_wise_by_cluster as t left join ingestion.dimension_cluster as c on t.cluster_id = c.cluster_id left join ingestion.dimension_master as m on t.cluster_id = m.cluster_id where m.block_id={block_id} group by t.cluster_id,cluster_name",
                    },
                    "level": "cluster"
                }
            },
            {
                ...
                "hierarchyLevel": "4",
                "timeSeriesQueries": {
                    "barChart": "select school_name as location, round(avg(percentage),2) as percentage from ingestion.sac_stds_avg_atd_grade_wise_by_school as t left join ingestion.dimension_school as s on t.school_id = s.school_id left join ingestion.dimension_master as m on t.school_id = m.school_id where (date between startDate and endDate) and m.cluster_id={cluster_id} group by t.school_id,school_name",
                },
                "query": "select t.cluster_id, cluster_name from ingestion.sac_stds_avg_atd_grade_wise_by_cluster as t left join ingestion.dimension_cluster as c on t.cluster_id = c.cluster_id left join ingestion.dimension_master as m on t.cluster_id = m.cluster_id where m.block_id={block_id} group by t.cluster_id,cluster_name",
                "actions": {
                    "queries": {
                        "barChart": "select min(date) as min_date, max(date) as max_date, school_name as location, round(avg(percentage),2) as percentage from ingestion.sac_stds_avg_atd_grade_wise_by_school as t left join ingestion.dimension_school as s on t.school_id = s.school_id left join ingestion.dimension_master as m on t.school_id = m.school_id where m.cluster_id={cluster_id} group by t.school_id,school_name",
                    },
