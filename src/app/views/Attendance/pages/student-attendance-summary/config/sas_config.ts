export const config = {
    average_attendance: {
        ...
        "timeSeriesQueries": {
            "table": "select min(date) as min_date, max(date) as max_date, state_name, round(avg(percentage),2) as percentage from ingestion.sac_stds_avg_atd_by_state as t left join ingestion.dimension_state as s on t.state_id = s.state_id where date between startDate and endDate group by t.state_id ,state_name",
