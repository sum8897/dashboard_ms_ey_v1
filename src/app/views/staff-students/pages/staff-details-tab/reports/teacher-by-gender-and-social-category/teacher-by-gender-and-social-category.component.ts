...
@Component({
  selector: 'app-teacher-by-gender-and-social-category',
  templateUrl: './teacher-by-gender-and-social-category.component.html',
  styleUrls: ['./teacher-by-gender-and-social-category.component.scss']
})
export class TeacherByGenderAndSocialCategoryComponent implements OnInit {
     ...
     if (this.drillDownDetails !== undefined) {
      ...
      Object.keys(queries).forEach(async (key: any) => {
        ...
        if (query && key === 'table'&& filterneed) {
          ...
          this._dataService.stackBar(reportData,config,"female","Female",'rgba(0, 255, 0, 0.5)',0.4);
          this._dataService.stackBar(reportData,config,"transgender","Transgender",'rgba(214,215,39, 0.6)',0.4);
  ...
  getBarChartReportData(query, options, filters, defaultLevel): void {
    ...
    this._commonService.getReportDataNew(query).subscribe((res: any) => {
      ...
      // this._dataService.stackBar(this.tableReportData,this.config,"primary_school","Primary",'rgba(0, 0, 255, 0.5)',0.6);
          ...
          this._dataService.stackBar(this.tableReportData,this.config,"female","Female",'rgba(0, 255, 0, 0.5)',0.4);
          this._dataService.stackBar(this.tableReportData,this.config,"transgender","Transgender",'rgba(214,215,39, 0.6)',0.4);
