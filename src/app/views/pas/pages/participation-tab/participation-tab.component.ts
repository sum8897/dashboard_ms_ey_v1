...
import { config } from 'src/app/views/pas/config/pas_config';
import { SchoolParticipationTableComponent } from './reports/school-participation-table/school-participation-table.component';
import { StudentParticipationTableComponent } from './reports/student-participation-table/student-participation-table.component';
 import { QuestionWiseBarchartComponent } from './reports/question-wise-barchart/question-wise-barchart.component';
...
export class ParticipationTabComponent implements OnInit, OnDestroy {
...
bigNumberMetrics: any = [];
  ...
  //
  @ViewChild('schoolParticipationTable') schoolParticipationTable: SchoolParticipationTableComponent;

  @ViewChild('questionWiseBarchart') questionWiseBarchart: QuestionWiseBarchartComponent;
 
  @ViewChild('studentParticipationTable') studentParticipationTable: StudentParticipationTableComponent;
  ...
  async ngAfterViewInit(): Promise<void> {
      if (this.hasCommonFilters) {
          ...
          this.schoolParticipationTable?.getReportData({filterneed: this.hasCommonFilters, filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) },this.startDate,this.endDate);

          this.studentParticipationTable?.getReportData({filterneed: this.hasCommonFilters, filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) },this.startDate,this.endDate);

       
        
         
          this.questionWiseBarchart?.getReportData({filterneed: this.hasCommonFilters, filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) },this.startDate,this.endDate);
