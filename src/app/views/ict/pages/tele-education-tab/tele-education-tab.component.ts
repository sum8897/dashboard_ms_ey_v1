...
@Component({
  selector: 'app-tele-education-tab',
  templateUrl: './tele-education-tab.component.html',
  styleUrls: ['./tele-education-tab.component.scss']
})
export class TeleEducationTabComponent implements OnInit, OnDestroy {
...
hasCommonFilters: boolean = true;
  ...
  @ViewChild('teleEducationTable') teleEducationTable: TeleEducationTableComponent;
  @ViewChild('teleCompleteBigNumber') teleCompleteBigNumber: TeleCompleteBignumberComponent;
  @ViewChild('teleNonCompleteBigNumber') teleNonCompleteBigNumber: TeleNoncompleteBignumberComponent;
  ...
  async ngAfterViewInit(): Promise<void> {
      if (this.hasCommonFilters) {
          ...
          this.teleEducationTable?.getReportData({filterneed: this.hasCommonFilters, filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) },this.startDate,this.endDate);
        
          this.teleCompleteBigNumber?.getReportData({filterneed: this.hasCommonFilters, filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) },this.startDate,this.endDate);

          this.teleNonCompleteBigNumber?.getReportData({filterneed: this.hasCommonFilters, filterValues: this.filters.map((filter) => { return { ...filter, columnName: filter.valueProp, filterType: filter.id } }) },this.startDate,this.endDate);
