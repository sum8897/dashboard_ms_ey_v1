...
export class ReportDrilldownService {
  ...
  async drilldown(
    ...
    console.log(prevDrillDownDetails);
    ...
    for (let i = 0; i < types.length; i++) {
      ...
      if (query && types[i] === "table") {
        ...
        reportData = await this._dataService.getBarChartReportData(
          query,
          options,
          filters,
          defaultLevel
        ...
        reportData = await await this._dataService.getMapReportData(
          query,
          options,
          matric_filter ? matric_filter : undefined
        );
